...
function guestbook_get_msgs($page = 0, $parse = true, $limited = true, $file = RCMS_GB_DEFAULT_FILE, $config = 'guestbook.ini') {
	...
	if(!isset($data)) {
		if (!is_readable($file) || !($data = unpack_data(file_get_contents($file)))) $data = array();
	}
	if(!empty($data)){
		$c    = sizeof($data);
		$ndata = $rdata = array();
...
function guestbook_post_msg($username, $nickname, $text, $file = RCMS_GB_DEFAULT_FILE, $config = 'guestbook.ini') {
	...
	$newmesg['nickname'] = (!empty($config['max_word_len']) && strlen($nickname) > $config['max_word_len']) ? '<abbr title="' . hcms_htmlsecure($nickname) . '">' . mb_substr($nickname, 0, $config['max_word_len']) . '</abbr>' : hcms_htmlsecure($nickname);
	$newmesg['time'] = rcms_get_time();
	$newmesg['text'] = (strlen($text) > $config['max_message_len']) ? mb_substr($text, 0, $config['max_message_len']) : $text;
	$data[] = $newmesg;
	return file_write_contents($file, pack_data($data));
}
