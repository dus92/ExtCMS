...
class polls{
    ...
    function openCurrentPolls(){
        ...
        if(file_exists($this->polls_file)){
            if(($file = unpack_data(file_get_contents($this->polls_file))) === false) {
    ...
    function openArchivedPolls(){
        ...
        if(file_exists($this->old_polls_file)){
            if(!($file = unpack_data(file_get_contents($this->old_polls_file)))) {
    ...
    function close($uc = true, $uo = true){
        if($uc) {
        	...
        	$a = file_write_contents($this->polls_file, pack_data($this->current));
        }
        if($uo) {
        	if(!$this->oopened) $this->openArchivedPolls();
        	$b = file_write_contents($this->old_polls_file, pack_data($this->old));
        }
        if($uc && $uo) {
        	return $a && $b;
        } elseif ($uo) {
