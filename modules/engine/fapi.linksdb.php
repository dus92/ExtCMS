...
class linksdb{
    ...
    function linksdb($file){
        $this->file = $file;
        if(is_readable($file) && $data = unpack_data(file_get_contents($file))) $this->data = $data;
    ...
    function createFile($cid, $name, $desc, $link, $size, $author, $license, $platform, $preview){
        if(empty($name)) return false;
        if(empty($this->data[$cid])) return false;
        $this->data[$cid]['files'][] = array('name' => $name, 'desc' => $desc, 'link' => $link, 'size' => $size, 'date' => rcms_get_time(),  'author' => $author, 'license' => $license, 'platform' => $platform, 'count' => 0, 'preview' => $preview);
        return true;
    }
    
    function updateFile($cid, $fid, $name, $desc, $link, $size, $author, $license, $platform, $preview){
        if(empty($name)) return false;
        if(empty($this->data[$cid])) return false;
        ...
        $this->data[$cid]['files'][$fid]['license'] = $license;
        $this->data[$cid]['files'][$fid]['platform'] = $platform;
        $this->data[$cid]['files'][$fid]['preview'] = $preview;
        return true;
    }
    ...
    function close(){
        ...
        if(!($data = pack_data($res))) return false;
