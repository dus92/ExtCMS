...
function rcms_parse_ini_file($filename, $blocks = false){
	...
	foreach ($array1 as $filedata) {
		...
		if ($firstchar != ';' && !empty($dataline)) {
			if ($blocks && $firstchar == '[' && mb_substr($dataline, -1, 1) == ']') {
				$section = strtolower(mb_substr($dataline, 1, -1));
			} else {
...
function rcms_scandir($directory, $exp = '', $type = 'all', $do_not_filter = false) {
	...
	if(is_dir($directory)){
		...
		while (false !== ($filename = readdir($fh))) {
			if(mb_substr($filename, 0, 1) != '.' || $do_not_filter) {
...
function rcms_get_current_id($directory, $ending) {
	...
	$endfile = @end($files);
	$current = mb_substr($endfile, 0, mb_strlen($endfile)-mb_strlen($ending));
	$current +=1;
...
{
	return str_replace(array('ё','ж','ц','ч','ш','щ','ю','я','Ё','Ж','Ц','Ч','Ш','Щ','Ю','Я','А','Б','В','Г','Д','Е','З','И','Й','К','Л','М','Н','О','П','Р','С','Т','У','Ф','Х','Ъ','Ы','Ь','Э','а','б','в','г','д','е','з','и','й','к','л','м','н','о','п','р','с','т','у','ф','х','ъ','ы','ь','э'),array('yo','zh','z','ch','sh','sh','yu','ya','YO','ZH','TC','CH','SH','SH','YU','YA','A','B','V','G','D','E','Z','I','J','K','L','M','N','O','P','R','S','T','U','F','H','_','I','_','E','a','b','v','g','d','e','z','i','j','k','l','m','n','o','p','r','s','t','u','f','h','','i','','e'),$string);
}
?>