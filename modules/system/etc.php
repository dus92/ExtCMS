...
function rcms_htmlspecialchars_recursive($array) {
	foreach ($array as $key =>$value){
		...
		else $array[$key] = hcms_htmlsecure($value);
	}
	return $array;
}
/**
...
function rcms_send_mail($to, $from, $sender, $encoding, $subj, $text) {
	...
	$headers .= "X-Mailer: Hakkah~CMS\n";
	$headers .= "X-MimeOLE: Hakkah~CMS\n";
...
function rcms_format_time($format, $gmepoch, $tz = ''){
	...
	return ( !empty($translate) ) ? strtr(@gmdate($format, $gmepoch), $translate) : @gmdate($format, $gmepoch);
}
...
function rcms_parse_text_by_mode($str, $mode){
	$str = str_replace(']./',']'.hcms_get_weblocation(),$str); // wtf?
	switch ($mode){
		default:
		case 'check':
...
class message{
	...
	function parseMultilangTag(){
		global $system;
		preg_match_all("#[\s\n\r]*\[multilang(=(\"|&quot;|)(.*?)(\"|&quot;|)|)\][\s\n\r]*(.*?)[\s\n\r]*\[/multilang\][\s\n\r]*#is", $this->str, $matches1, PREG_SET_ORDER);
		foreach($matches1 as $match1) {
			$default = (!empty($match1[3])) ? $match1[3] : $system->config['default_lang'];
			$lngblocks = array();
			$data = $match1[5];
			preg_match_all("#[\s\n\r]*\[lang=(\"|&quot;|)(.*?)(\"|&quot;|)\][\s\n\r]*(.*?)[\s\n\r]*\[/lang\][\s\n\r]*#is", $data, $matches2, PREG_SET_ORDER);
			foreach($matches2 as $match2) {
				$lngblocks[$match2[2]] = $match2[4];
			}
			if(!empty($lngblocks)){
				$lng = (!empty($lngblocks[$system->language])) ? $system->language : $default;
				list($fstlng, $_null) = each($lngblocks);
				$lng = (!empty($lngblocks[$lng])) ? $lng : $fstlng;
				
				$this->str = str_replace($match1[0], $lngblocks[$lng], $this->str);
			} else {
				$this->str = str_replace($match1[0], '', $this->str);
			}
		}
		return true;
	}
	...
	/**
     * Makes all URLs in message::str clickable
     *
     */
	function parseUrls(){
...
function rcms_prc_link($matches){
	if(mb_strlen($matches[2]) > 25){
		return ' <a href="' . $matches[2] . '" target="_blank">' . mb_substr($matches[2], 0, 11) . '...' . mb_substr($matches[2], -11) . '</a>';
...
function rcms_spoiler_tag($matches){
	...
	if(!empty($matches[3])) $title = __('Spoiler') . ': ' . $matches[3]; else $title = __('Spoiler') . ' (' . __('click to view') . ')';
