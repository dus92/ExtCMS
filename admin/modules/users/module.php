...
if($system->checkForRight('USERS')) {
    $MODULES[$category][1]['categories'] = __('Manage group categories');
    $MODULES[$category][1]['groups'] = __('Manage groups');
    $MODULES[$category][1]['fields'] = __('Additional profile fields');
    $MODULES[$category][1]['create'] = __('Create user');
    $MODULES[$category][1]['profiles'] = __('Manage users');
	$MODULES[$category][1]['bans'] = __('Manage bans');
}
else if($system->checkForRight('USERS-MANAGER'))
{
	$MODULES[$category][1]['create'] = __('Create user');
    $MODULES[$category][1]['profiles'] = __('Manage users');	
}
