...
if (!empty($_POST['backupit'])) {
    ...
    $backup->addDirectory('content/a', true);
    $backup->addDirectory('content/datafiles', true);
    $backup->addDirectory('content/logs', true);
    $backup->addDirectory('content/rss_cache', true);
    $backup->addDirectory('content/users', true);
    $backup->addFile('content/meta_tags.html');
    
    if(!empty($_POST['back_gallery']))
    	$backup->addDirectory('content/gallery', true);
    if(!empty($_POST['back_gallery']))
		$backup->addDirectory('content/uploads', true);
...
$frm->addrow(__('Include "content/gallery" folder'), $frm->checkbox('back_gallery', '1', '', true));
$frm->addrow(__('Include "content/uploads" folder'), $frm->checkbox('back_uploads', '1', '', true));
$frm->show();
?>
