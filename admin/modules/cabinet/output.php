...
$id = $iblockitem->GetTableAINextValue ();
    ...
    if (empty($_POST['mess_output_click']) )//отображение писем
    {        
        ...
        $frm->addsingle('<tr id="tr_output_header"><th colspan="4">Отправленные</th></tr>');
    ...
    else if (!empty($_POST['mess_output_click'])) //подробный просмотр отправленных сообщений
    {
        ...
        while ( $to = $imail->Read ( $resultid ) ) 
        {
           ...
           // {
                ...
                //$frm->addrow(__('Message'), iconv('utf-8', 'cp1251', unpack_data ( $to ['message'] )));
        ...
        print('<script type="text/javascript">
            $(document).ready(function()
            {
                ...
                {                   
                   //history.back();
                   //return false; 
                   $("form[name=frm_output]").attr("action", "admin.php?show=module&id=cabinet.output"); 
