...
Ext.define('Ext.ux.upload.Queue', {
    ...
    getFirstReadyItem : function() {
        ...
        for (i = 0; i < num; i++) {
			if (items[i].isReady() && !this.validateFile(null, items[i].getSize(), items[i].getExt()).error) {
                return items[i];
            }
        }
     ...
     */
    removeItemsByKey : function(itemKeys, panel) {
        var i;
		...
		me.each(function(item, index, items) {
            if (Ext.Array.contains(itemKeys, item.getFilename()) && item.getUrl() != '') {
        		ajaxRequest(panel, {
	            	url: 'fileDelete',
	            	params: {
	            		url: item.getUrl()	
	            	...
	            	success: function(res){
						me.removeItemByKey(item.getFilename());
						me.fireEvent('queuechange', me);
	            	}
	            }, 0);
            }
            else if(item.getUrl() == '' || !item.getUrl()){
            	me.removeItemByKey(item.getFilename());
				me.fireEvent('queuechange', me);
            }
        });
    ...
    createItem : function(file) {
        ...
        if(this.validateFile(null, item.getSize(), item.getExt()).error){
        	//item.setStatus(item.STATUS_UPLOAD_ERROR);
        	this.status = item.STATUS_UPLOAD_ERROR;
        }
        else{
